...
            DECLARE @applicationstageid INT, @applicationid INT, @paymenttransactionid INT, @timediffrence INT, @returnmessage VARCHAR(50), @paid BIT, @openstatus INT = 1, @closestatus INT = 2,                   
   @stageid INT, @serviceid INT, @statusId INT, @uId INT, @IsPermission INT = 1, @serviceTranslationKeyId INT = 11, @englanguageid INT = 1, @arabiclanguageid INT = 2, @nextstageavailability AS BIT= 0, @stageActionId AS INT, @appStageActionId AS INT ;     
...
            WHERE AAS.Id = (Select ApplicationStageId from application.ApplicationStageActions Where Id = @appStageActionId)                         
                                 
      
-- Checked new stage availability                  
    Select @stageActionId = StageActionId from application.ApplicationStageActions Where Id = @appStageActionId               
