...
            DECLARE @applicationstageid INT, @applicationid INT, @paymenttransactionid INT, @timediffrence INT, @returnmessage VARCHAR(50), @paid BIT, @openstatus INT = 1, @closestatus INT = 2,     
   @stageid INT, @serviceid INT, @statusId INT, @uId INT, @IsPermission INT = 1, @serviceTranslationKeyId INT = 11, @englanguageid INT = 1, @arabiclanguageid INT = 2, @nextstageavailability AS BIT= 0, @stageActionId AS INT ;           
                
...
-- Checked new stage availability		  
		  Select TOP 1 @stageActionId = StageActionId from application.ApplicationStageActions Where ApplicationStageId = @applicationstageid ORDER BY 1 DESC
